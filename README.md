#  `BASES DE DATOS (EST - 230)`

## `Objetivo General`
> Diseñar e implementar una Base de Datos aplicando herramientas conceptuales (Modelos de Datos) y computacionales (Sistemas Gestores de Bases de Datos, Lenguajes de Programación y Lenguajes de Datos).


## `Contenido Temático`
> - Tema 1 : Modelo de Datos
> - Tema 2 : Modelo Entidad - Relación
> - Tema 3 : Modelo Relacional
> - Tema 4 : Algebra Relacional
> - Tema 5 : Lenguaje SQL 
> - Tema 6 : Lenguaje R y Bases de Datos


## `Bibliografía`
* Ricardo, Catherine M., (2009). “Bases de Datos”, Editorial McGraw Hill Interamericana Editores S.A., México.
* Silberschutz, Kort y Sudarstran. Fundamentos de Base de Datos. Editorial McGrawHill.
* Adoración de M. y Piattini M.  Fundamentos y Modelos de Base de Datos. Editorial Alfaomerga-Rama.
* Adoración de M. y Piattini M. Diseño de Base de Datos Relacionales. Editorial Alfaomega-Rama.
* Adoración de M. y Piattini M. Diseño de Base de Datos- Problemas resueltos.  Editorial Alfaomerga-Rama.


## `Evaluación`
| Examen | Ponderación| Fecha  |  Temas  |
| ---------------- |:-------------:| ---------------- | ---------------- |
| Primer Parcial	|    25 | 6/10/2020    | 1 y 2 |
| Segundo Parcial   	|    25 | 24/11/2020   | 3 y 4 |
| Tercer Parcial     	|    30 | 10/12/2020   | 5 y 6 |
| Prácticas             |    20 |     |       |


## `Cronograma Entrega de Prácticas`
| Práctica | Evaluación | Fecha |
| ---------------- |:-------------:| ---------------- |
| 1	|4 	| 15/09/2020     |
| 2	|4	| 06/10/2020     |
| 3	|4	| 27/10/2020     |
| 4	|4	| 24/11/2020     |
| 5	|4	| 10/12/2020     |

Nota. Las prácticas deberán ser elaboradas en un procesador de textos (Word) o Sistema Tipográfico (LaTeX). 

## `Video / Video Conferencia por Google Meet`
| Tema |  Fecha  |  
| ---------------- | ---------------- | 
| 1    | 01/9/2020   | 
| 2    | 15/9/2020   | 
| 3    | 13/10/2020   |   
| 4    | 10/11/2020 -> | 


### `Examen Recuperatorio`
>  17 de diciembre : Temas correspondientes al parcial

### `Publicación de Notas Finales
>  24 de diciembre
